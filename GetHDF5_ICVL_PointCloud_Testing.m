...
%% load data
txtFilePath = 'C:/Users/Clarissa/Desktop/Projekt/ICVL/Testing/';
temp = importdata([txtFilePath, 'test_seq_2.txt']);
...
tic; %start the timer
for ind=700:700
   %seq1: 702 seq2: 894  should it be, now just test with one sample
    ...
    COM_GroundTruth = mean(GroundTruthXYZ,2);
    
    allUs = ones(240,1)*(1:320);
    allVs = (1:240)' * ones(1,320);
    allDs = image;
    
    maxDGroundtruth= max(squeeze(GroundTruthUVDs(ind,3,:)));

   validMaskD = ((allDs<(maxDGroundtruth+60)) );
  
    allDs = allDs(validMaskD);   allUs = allUs(validMaskD);     allVs = allVs(validMaskD); 

    
    [allX, allY, allZ] = uvd2xyz(allUs(:), allVs(:), allDs(:)); % allUs(:) means vecotrize a matrix [320,240]-->[76800]
    pointCloud = [allX, allY, allZ]';
      pointCloud = pointCloud(:, (abs(pointCloud(1,:)- COM_GroundTruth(1,:)))<160);     %filter x
      pointCloud = pointCloud(:, (abs(pointCloud(2,:)- COM_GroundTruth(2,:)))<160);     %filter y 
      pointCloud = pointCloud(:, (abs(pointCloud(3,:)- COM_GroundTruth(3,:)))<160);     %filter z

    pointCloud = pointCloud/1000;  %mm to m conversion
    GroundTruthXYZ = GroundTruthXYZ/1000; %mm to m conversion
    COM_GroundTruth = COM_GroundTruth/1000;
   ...
   hold on;
    scatter3(GroundTruthXYZ(1,:), GroundTruthXYZ(2,:), GroundTruthXYZ(3,:), 80,'filled','r');
    hold on;
    scatter3(COM_GroundTruth(1,:), COM_GroundTruth(2,:), COM_GroundTruth(3,:), 80,'filled','y');%visualize the ground truth pose in red size 80
    view([0,0,-1])
...
%     %
    ...
    if (mod(ind,h5fileSize)==0 || ind==894)
        toc
        savePath = 'C:/Users/Clarissa/Desktop/Projekt/ICVL/pointdata/Testing/';
        h5filename = [savePath,'ICVLTestPointCloud_seq2_',num2str(ceil(ind/h5fileSize)),'.h5'];
        h5filename
        delete(h5filename);
